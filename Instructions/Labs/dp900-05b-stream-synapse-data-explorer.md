...
1. Select **Data Explorer pools**, and then use the **&#65291; New** icon to create a new pool with the following settings:
    - **Data Explorer pool name**: *Enter an appropriate name, for example "dxpool<your_name>"*.
...
1. Select **Review and create** and then select **Create** to create the Data Explorer pool, and then wait for it to be deployed (which may take 15 minutes or longer - the status will change from *Creating* to *Online*).

## Create a database and ingest data

1. In Synapse Studio, select the **Data** page.

1. Ensure that the **Workspace** tab is selected, and if necessary, select the **&#8635;** icon at the top-right of the page to refresh the view so that **Data Explorer databases** is listed.

1. Expand **Data Explorer databases** and verify that **dxpool<your_name>** is listed.

1. In the **Data** pane, use the **&#65291;** icon to create a new **Data Explorer database** in the **dxpool<your_name>** pool with the name **iot-data**.
...
1. In the new browser tab containing Azure Data Explorer, on the **Data** tab, select **Ingest data**.

1. In the **Destination** page, select the following settings:
    - **Cluster**: *The **dxpool<your_name>** Data Explorer pool in your Azure Synapse workspace*
    - **Database**: iot-data
